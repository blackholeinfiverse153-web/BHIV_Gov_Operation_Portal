...
export const addCitizen = (citizen: Omit<CitizenData, "id">) =>
  http<CitizenData>("/api/citizens", {
    method: "POST",
    body: JSON.stringify(citizen),
  });

export const updateCitizen = (citizen: CitizenData) =>
  http<CitizenData>(`/api/citizens/${citizen.id}`, {
    method: "PUT",
    body: JSON.stringify(citizen),
  });
...
export const addOfficer = (officer: Omit<OfficerData, "id">) =>
  http<OfficerData>("/api/officers", {
    method: "POST",
    body: JSON.stringify(officer),
  });

export const updateOfficer = (officer: OfficerData) =>
  http<OfficerData>(`/api/officers/${officer.id}`, {
    method: "PUT",
    body: JSON.stringify(officer),
  });
...
export const addRequest = (request: Omit<RequestData, "id">) =>
  http<RequestData>("/api/requests", {
    method: "POST",
    body: JSON.stringify(request),
  });

export const updateRequest = (request: RequestData) =>
  http<RequestData>(`/api/requests/${request.id}`, {
    method: "PUT",
    body: JSON.stringify(request),
  });
...
export const setRequestStatus = (id: string, status: RequestStatus) =>
  http<RequestData>(
    `/api/requests/${id}/status?status=${encodeURIComponent(status)}`,
    { method: "PATCH" },
  );

export const deleteRequest = (requestId: string) =>
  http<{ deleted: string }>(`/api/requests/${requestId}`, { method: "DELETE" });
...
export const addProject = (project: Omit<ProjectData, "id">) =>
  http<ProjectData>("/api/projects", {
    method: "POST",
    body: JSON.stringify(project),
  });

export const updateProject = (project: ProjectData) =>
  http<ProjectData>(`/api/projects/${project.id}`, {
    method: "PUT",
    body: JSON.stringify(project),
  });
